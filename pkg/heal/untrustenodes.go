...
// are removed from the cluster (with the "forget nodes" command) but try to rejoin the cluster.
	...
	for _, currentPod := range podlist {
		if currentPod.Name == node.PodName {
			...
			if currentPod.Status.PodIP == node.IP {
				// this check is used to see if the Pod name is not use by another RedisNode.
				// for that we check the Pod name from the Redis node is not used by another
