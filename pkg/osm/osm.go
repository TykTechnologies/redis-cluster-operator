...
import (
	...
	otx "github.com/TykTechnologies/redis-cluster-operator/pkg/osm/context"
	...
	if spec.S3 != nil {
		...
		if spec.S3.Endpoint == "" || strings.HasSuffix(spec.S3.Endpoint, ".amazonaws.com") {
			...
			out, err := svc.GetBucketLocation(&_s3.GetBucketLocationInput{
				Bucket: StringP(spec.S3.Bucket),
			})
			if err != nil {
				return nil, err
			}
			nc.Config[s3.ConfigRegion] = stringz.Val(String(out.LocationConstraint), "us-east-1")
		} else {
