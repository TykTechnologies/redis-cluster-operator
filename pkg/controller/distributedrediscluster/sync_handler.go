...
import (
	...
	"github.com/TykTechnologies/redis-cluster-operator/pkg/clustering"
	"github.com/TykTechnologies/redis-cluster-operator/pkg/config"
	"github.com/TykTechnologies/redis-cluster-operator/pkg/controller"
	"github.com/TykTechnologies/redis-cluster-operator/pkg/k8sutil"
	"github.com/TykTechnologies/redis-cluster-operator/pkg/manager"
	"github.com/TykTechnologies/redis-cluster-operator/pkg/redisutil"
	"github.com/TykTechnologies/redis-cluster-operator/pkg/resources/statefulsets"
...
func (r *DistributedRedisClusterReconciler) ensureCluster(ctx *syncContext) error {
	...
	if err := r.validateAndSetDefault(cluster, ctx.reqLogger); err != nil {
		if k8sutil.IsRequestRetryable(err) {
			return controller.Kubernetes.Wrap(err, "Validate")
		}
		return controller.StopRetry.Wrap(err, "stop retry")
	}

	// Redis only load db from append only file when AOF ON, because
	...
	if err := r.ensurer.EnsureRedisConfigMap(cluster, labels); err != nil {
		return controller.Kubernetes.Wrap(err, "EnsureRedisConfigMap")
	}

	if err := r.resetClusterPassword(ctx); err != nil {
		return controller.Cluster.Wrap(err, "ResetPassword")
	}

	if updated, err := r.ensurer.EnsureRedisStatefulsets(cluster, labels); err != nil {
		ctx.reqLogger.Error(err, "EnsureRedisStatefulSets")
		return controller.Kubernetes.Wrap(err, "EnsureRedisStatefulSets")
	} else if updated {
	...
	if err := r.ensurer.EnsureRedisHeadLessSvcs(cluster, labels); err != nil {
		return controller.Kubernetes.Wrap(err, "EnsureRedisHeadLessSvcs")
	}
	if err := r.ensurer.EnsureRedisSvc(cluster, labels); err != nil {
		return controller.Kubernetes.Wrap(err, "EnsureRedisSvc")
	}
	if err := r.ensurer.EnsureRedisRCloneSecret(cluster, labels); err != nil {
		if k8sutil.IsRequestRetryable(err) {
			return controller.Kubernetes.Wrap(err, "EnsureRedisRCloneSecret")
		}
		return controller.StopRetry.Wrap(err, "stop retry")
	}
	return nil
...
func (r *DistributedRedisClusterReconciler) waitPodReady(ctx *syncContext) error {
	if _, err := ctx.healer.FixTerminatingPods(ctx.cluster, 5*time.Minute); err != nil {
		return controller.Kubernetes.Wrap(err, "FixTerminatingPods")
	}
	if err := r.checker.CheckRedisNodeNum(ctx.cluster); err != nil {
		return controller.Requeue.Wrap(err, "CheckRedisNodeNum")
	}
...
func (r *DistributedRedisClusterReconciler) waitForClusterJoin(ctx *syncContext) error {
	...
	if err != nil {
		return controller.Redis.Wrap(err, "AttachNodeToCluster")
	}
	...
	if err != nil {
		return controller.Requeue.Wrap(err, "wait for cluster join")
	}
	return nil
}
...
func (r *DistributedRedisClusterReconciler) syncCluster(ctx *syncContext) error {
	...
	if err != nil {
		return controller.Cluster.Wrap(err, "newRedisCluster")
	}
	...
	if err := clusterCtx.DispatchMasters(); err != nil {
		return controller.Cluster.Wrap(err, "DispatchMasters")
	}
	...
	if len(curMasters) == 0 {
		...
		if err := clusterCtx.PlaceSlaves(); err != nil {
			return controller.Cluster.Wrap(err, "PlaceSlaves")

		}
		if err := clusterCtx.AttachingSlavesToMaster(admin); err != nil {
			return controller.Cluster.Wrap(err, "AttachingSlavesToMaster")
		}

		if err := clusterCtx.AllocSlots(admin, newMasters); err != nil {
			return controller.Cluster.Wrap(err, "AllocSlots")
		}
		...
		if err := clusterCtx.RebalancedCluster(admin, newMasters); err != nil {
			return controller.Cluster.Wrap(err, "RebalancedCluster")
		}
...
func (r *DistributedRedisClusterReconciler) scalingDown(ctx *syncContext, currentMasterNum int, statefulSetNodes map[string]redisutil.Nodes) error {
	...
	for i := currentMasterNum - 1; i >= expectMasterNum; i-- {
		...
		if err != nil {
			return controller.Kubernetes.Wrap(err, "GetStatefulSet")
		}
		for _, node := range statefulSetNodes[stsName] {
			ctx.reqLogger.Info("forgetNode", "id", node.ID, "ip", node.IP, "role", node.GetRole())
			if len(node.Slots) > 0 {
				return controller.Redis.New(fmt.Sprintf("node %s is not empty! Reshard data away and try again", node.String()))
			}
			if err := admin.ForgetNode(node.ID); err != nil {
				return controller.Redis.Wrap(err, "ForgetNode")
			}
		}
		// remove resource
...
func (r *DistributedRedisClusterReconciler) resetClusterPassword(ctx *syncContext) error {
	if err := r.checker.CheckRedisNodeNum(ctx.cluster); err == nil {
