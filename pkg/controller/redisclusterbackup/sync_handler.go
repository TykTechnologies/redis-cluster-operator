...
import (
	...
	"github.com/TykTechnologies/redis-cluster-operator/pkg/event"
	"github.com/TykTechnologies/redis-cluster-operator/pkg/k8sutil"
	"github.com/TykTechnologies/redis-cluster-operator/pkg/osm"
	"github.com/TykTechnologies/redis-cluster-operator/pkg/utils"
...
func (r *RedisClusterBackupReconciler) create(reqLogger logr.Logger, backup *redisv1alpha1.RedisClusterBackup) error {
	...
	if err := r.ValidateBackup(backup); err != nil {
		...
		err := r.markAsFailedBackup(backup, err.Error())
		if err != nil {
			return err
		}
		r.recorder.Event(
	...
	if err != nil {
		...
		err := r.markAsFailedBackup(backup, msg)
		if err != nil {
			return err
		}
		r.recorder.Event(
...
func (r *RedisClusterBackupReconciler) handleBackupJob(reqLogger logr.Logger, backup *redisv1alpha1.RedisClusterBackup) error {
	...
	if err != nil {
		...
		if errors.IsNotFound(err) {
			...
			err := r.markAsIgnoredBackup(backup, msg)
			if err != nil {
				return err
			}
			delete(backup.GetLabels(), redisv1alpha1.LabelBackupStatus)
